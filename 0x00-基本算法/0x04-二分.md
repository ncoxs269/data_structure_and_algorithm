...
假设最优解的评分是 $S$，显然对于 $\forall x >S$, 都不存在一个合法的方案达到 $x$ 分，否则就与 $S$ 的最优性矛盾；而对于 $\forall x \le S$，一定存在一个合法的方案达到或超过 x 分，因为最优解就满足这个条件。这样问题的值域就具有一种特殊的单调性—在 $S$ 的一侧合法、在 $S$ 的另一侧不合法，就像一个在 $(-\infin, S]$上值为 1，在 $(S,+\infin)$ 上值为 0 的分段函数，可通过二分找到这个分界点 $S$。
